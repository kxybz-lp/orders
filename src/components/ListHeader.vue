<template>
  <el-row :gutter="0" class="list-header">
    <el-col :span="10" :offset="0">
      <el-button v-if="rule.create && rule.create === 79" v-permission="rule.create" type="primary" size="default" @click="$router.push('/order/add')">新增</el-button>
      <el-button v-if="rule.create && rule.create !== 79" v-permission="rule.create" type="primary" size="default" @click="$emit('add')">新增</el-button>
      ...
      <el-button v-if="rule.delete && !$store.state.isMobile" v-permission="rule.delete" type="danger" size="default" @click="$emit('delete')">批量删除</el-button>
      ...
      <el-button v-if="rule.export && !$store.state.isMobile && $store.state.adminInfo.id != 781" v-permission="rule.export" type="danger" size="default" @click="$emit('export')">导出</el-button>
