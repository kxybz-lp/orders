  ...
  <div class="app-container">
    <el-card class="admin-card" shadow="hover">
      ...
      <el-table :data="dataList" stripe style="width: 100%" :header-cell-style="{ color: '#2c3e50', backgroundColor: '#f2f2f2' }" v-loading="loading" row-key="id">
        ...
        <el-table-column prop="name" label="名称" width="260">
          <template #default="scope">
            <div style="display: flex; align-items: center">
              <el-tag size="small" :type="scope.row.pid ? 'info' : ''">{{ scope.row.pid ? '来源' : '渠道' }}</el-tag>
        ...
        <el-table-column label="状态" width="120" v-permission="92">
          <template #default="scope">
        ...
        <el-table-column label="操作" width="260">
          <template #default="scope">
            <el-button v-permission="94" size="small" type="warning" @click="addChild(scope.row.id)">添加来源 </el-button>
            <el-button v-permission="91" size="small" type="primary" @click="handleEdit(scope.row)">编辑 </el-button>
            <el-button v-permission="95" size="small" type="danger" @click="handleDelete(scope.row.id)"> 删除 </el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
