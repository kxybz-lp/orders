        ...
        <el-tabs v-model="detailTab">
          <el-tab-pane label="订单信息" name="detail_order">
            <el-descriptions :column="1">
              ...
              <el-descriptions-item v-if="$store.state.adminInfo.branch_id === '1'" label="录入者：">
                {{ detail.user_name }} </el-descriptions-item>
              <el-descriptions-item v-if="$store.state.adminInfo.branch_id === '1'" label="录入时间：">
                {{ detail.create_time }} </el-descriptions-item>
            </el-descriptions>
          </el-tab-pane>
          <el-tab-pane label="派单信息" name="detail_arrange">
            <el-descriptions :column="1">
              <el-descriptions-item label="派单时间：">{{ parseTime(detail.arrange_time) }}
              </el-descriptions-item>
              <el-descriptions-item v-if="$store.state.adminInfo.branch_id === '1'" label="派单人：">
                {{ detail.arrange_man_as }}</el-descriptions-item>
          ...
          <el-tab-pane label="跟进信息" name="detail_follow">
            <el-descriptions :column="1">
              ...
              <el-descriptions-item v-if="detail.follows.length > 0">
                <el-descriptions style="margin-left: 0px;margin-top: -20px;" :column="1" border>
                  <template v-for="item in detail.follows" :key="item.id">
                    <el-descriptions-item label="跟进时间">{{ parseTime(item.follow_time) }}
                    </el-descriptions-item>
              ...
              <el-descriptions-item label="签约时间:">{{ parseTime(detail.signing_time) }}
              </el-descriptions-item>
              <el-descriptions-item label="合同金额：">{{ detail.contract_money }}</el-descriptions-item>
              <el-descriptions-item label="开工时间：">{{ parseTime(detail.start_time) }}
              </el-descriptions-item>
              <el-descriptions-item label="完工时间：">{{ parseTime(detail.end_time) }}
              </el-descriptions-item>
              <el-descriptions-item label="施工经理：">{{ detail.construction_manager }}
              </el-descriptions-item>
              <el-descriptions-item label="质检：">{{ detail.quality_man }}</el-descriptions-item>
            </el-descriptions>
          </el-tab-pane>
          <el-tab-pane v-if="$store.state.adminInfo.branch_id === '1'" label="回访信息"
            name="detail_visit">
            <el-descriptions :column="1">
